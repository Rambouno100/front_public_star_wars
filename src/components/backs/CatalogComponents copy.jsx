...
import { Search, ArrowUpDown, X, ChevronDown, Check, Settings } from 'lucide-react';
...
export const Checkbox = ({ checked, onClick, children, className = "" }) => (
...
export const FilterDropdown = ({ etiqueta, opciones, onChange, seleccionados = [] }) => {
...
export const ColumnSelector = ({ columnas, columnasVisibles, onChange }) => {
  const [abierto, setAbierto] = useState(false);
  const ref = useClickOutside(() => setAbierto(false));
...
export const CatalogView = ({ products, onViewDetail, cartHook, ProductActions }) => {
